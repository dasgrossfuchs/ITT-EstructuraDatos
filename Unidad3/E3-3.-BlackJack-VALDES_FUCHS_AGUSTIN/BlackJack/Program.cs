...
namespace BlackJack
{
    ...
    public class Blackjack
    {
        ...
        public void Barajear()//metodo que podria ser mas simple si le metiera todos los datos al arreglo manualmente pero nadie tiene tiempo para eso, imaginate cuanto tiempo me tardaria dandole copy paste a muchas cosas , oigame no ,aqui tengo un switch muy bonito
        {
            ...
            barajareada = new Queue(baraja);//la cola es inicializada con los valores de la baraja barajeada, esta se llama barajareada porque puedo
        }
        public void Hitme()//Me repartes una carta porfavor amiguito casa :)
        {
            Console.WriteLine("Carta No.{0}\t{1}\t",(indicemano+1), barajareada.Peek());
            char[] liposuccion = {' ','♥','♦','♣','♠'};
            
            string temp = string.Format("{0}",(barajareada.Peek().ToString()).Trim(liposuccion));//le quita el palo(*ouch*),esto se puede hacer en el switch pero es mas estetico con una variable local temporal y evita confusiones
